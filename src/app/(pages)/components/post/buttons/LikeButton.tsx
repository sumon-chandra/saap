...
const LikeButton: FC<LikeButtonProps> = ({ post }) => {
  ...
  const { data: likes, refetch: refetchLike } = useQuery<FullLikeTypes[]>({
    queryKey: ["like"],
    queryFn: async () => {
      return await axios.get(`/api/like`).then((response) => response.data);
    },
    staleTime: 60000 * 5,
    gcTime: 60000 * 10,
    ...
    <div className="flex items-center justify-start">
      <Button
        ...
        className={clsx(
          ...
          isLiked && "text-saap-secondary"
        )}
      >
        {isLiked ? <FaHeart /> : <FaRegHeart onClick={handleLike} />}
      </Button>
